<template>
  <VueChartJS type="radar" :options="chart_options" :data="chart_data" v-bind="chartProps()" />
</template>
...
export default {
  ...
  computed: {
    datasets(){ return _(this.iData).map((dataArray,name) => _.assign({},this.attrs,this.chartData(),_.zipObject(['label','borderColor','backgroundColor','data'],[name,this.getColor(_.indexOf(this.names,name),1),this.getColor(_.indexOf(this.names,name),_.toNumber(this.opacity || 0.2)),_.map(dataArray,'value')]))).value() },
    chart_data(){ return {
